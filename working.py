    ...
    if config.spinstop:
        if 'forts' in cell:
            for fort in cell['forts']:
                if 'type' in fort:
                    log.info('This is a PokeStop')
                    hack_chain=search_seen_fort(fort,api,position,config)
                    if hack_chain > 10:
                        log.info('need a rest')
                        break
                else:
                    log.info('This is a Gym')
...
def spawn_point_work(spawn_point,api,position):
    ...
    log.debug('Response dictionary 1: \n\r{}'.format(json.dumps(response_dict, indent=2)))
    time.sleep(2)
    api.player_update(latitude=lat,longitude=lng)
    response_dict = api.call()
    log.debug('Response dictionary 1: \n\r{}'.format(json.dumps(response_dict, indent=2)))
                ...
                if response_dict['responses']['ENCOUNTER']['status'] is 1:
                    ...
                    if 'wild_pokemon' in response_dict['responses']['ENCOUNTER']:
                        ...
                        if 'pokemon_data' in pokemon and 'cp' in pokemon['pokemon_data']:
                            ...
                            log.debug('A Wild ' + str(pokemon_name) + ' appeared! [CP' + str(cp) + ']')
                    while(True):
                        ...
                        if response_dict and \
                            ...
                            if status is 2:
                                log.debug('Attempted to capture %s - failed.. trying again!', pokemon_name)
                                time.sleep(1.25)
                                continue
                            if status is 1:
                                if cp < config.cp:
                                    log.info('Captured %s [CP%s]! - exchanging for candy', pokemon_name, cp)
                                    transfer_low_cp_pokomon(api,config.cp)
                                else:
                                    log.info('Captured %s [CP%s]!',pokemon_name, cp)
                        break
    time.sleep(5)
...
def search_seen_fort(fort,api,position,config):
    ...
    response_dict = api.call()
    log.debug('Response dictionary 2: \n\r{}'.format(json.dumps(response_dict, indent=2)))
    time.sleep(2)
    ...
    response_dict = api.call()
    log.debug('Response dictionary 3: \n\r{}'.format(json.dumps(response_dict, indent=2)))
    if 'responses' in response_dict and \
        ...
        if 'chain_hack_sequence_number' in response_dict['responses']['FORT_SEARCH']:
            ...
            log.debug('may search too often, lets have a rest')
            rest_time = 50
            return 11
    time.sleep(8)
    return 0
