...
ECHO.
ECHO. 2) Install 64-Bit Programs
ECHO.
ECHO. 3) Install PokemonGo-Bot
ECHO.
...
IF "%_ok%" == "1" SET CHOICE=32Bit&GOTO :32Bit
IF "%_ok%" == "2" SET CHOICE=64Bit&GOTO :64Bit
IF "%_ok%" == "3" SET CHOICE=GoBot&GOTO :GoBot
GOTO :_choice 
:32bit
...
cls
call:ech
set /p _Choose=Did you choose 32-Bit or 64-Bit Installation (1/2) ? 
if "%_Choose%" == "1" pip2 install --upgrade "%PGBotPath%\windows_bat\PyYAML-3.11-cp27-cp27m-win32.whl"
if "%_Choose%" == "2" pip2 install --upgrade "%PGBotPath%\windows_bat\PyYAML-3.11-cp27-cp27m-win_amd64.whl"
