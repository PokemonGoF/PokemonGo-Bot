...
class ApiRequest(PGoApiRequest):
    ...
    def can_call(self):
        ...
        if (self._position_lat is None) or (
                self._position_lng is None) or (self._position_alt is None):
    ...
    def is_response_valid(self, result, request_callers):
        ...
        # but still be wrong. we need to check if the server did sent what we
        # asked it
    ...
    def throttle_sleep(self):
        ...
        difference = now_milliseconds - \
            (self.last_api_request_time if self.last_api_request_time else 0)
