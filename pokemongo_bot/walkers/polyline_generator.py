...
class Polyline(object):
    ...
    def __init__(self, origin, destination, speed):
        ...
        self.URL = '{}&origin={}&destination={}'.format(
        ...
        try:
            self.polyline_points = [x['polyline'][
                'points'] for x in self.request_responce['routes'][0]['legs'][0]['steps']]
        except IndexError:
            self.polyline_points = self.request_responce['routes']
        self.speed = float(speed)
        self.points = [self.origin] + \
            self.get_points(self.polyline_points) + [self.destination]
    ...
    def get_points(self, polyline_points):
        ...
        crd_points = [x for n, x in enumerate(
            crd_points) if x not in crd_points[:n]]
        return crd_points
    ...
    def walk_steps(self):
        if self.points:
            ...
            walk_steps = filter(
                None, [
                    (o, d) if o != d else None for o, d in walk_steps])
    ...
    def get_pos(self):
        ...
        time_passed_distance = self.speed * \
            abs(time_passed - self._timestamp - self._paused_total)
        ...
        if self.walk_steps():
            ...
            for walked_end_step in sorted(steps_dict.keys()):
                if walked_end_step >= time_passed_distance:
                    break
            step_distance = haversine.haversine(
                *steps_dict[walked_end_step]) * 1000
            if walked_end_step >= time_passed_distance:
                percentage_walked = (
                    time_passed_distance - (walked_end_step - step_distance)) / step_distance
            else:
                percentage_walked = 1.0
            return self.calculate_coord(
                percentage_walked, *steps_dict[walked_end_step])
        else:
            # otherwise return the destination
            # https://github.com/th3w4y/PokemonGo-Bot/issues/27
    ...
    def get_total_distance(self):
        return ceil(
            sum([haversine.haversine(*x) * 1000 for x in self.walk_steps()]))
