...
class NicknamePokemon(BaseTask):
    ...
    def initialize(self):
        self.template = self.config.get(
            'nickname_template', '').lower().strip()
        if self.template == "{name}":
            self.template = ""

    def work(self):
        try:
            inventory = reduce(dict.__getitem__,
                               ["responses",
                                "GET_INVENTORY",
                                "inventory_delta",
                                "inventory_items"],
    ...
    def _get_inventory_pokemon(self, inventory_dict):
        ...
        for inv_data in inventory_dict:
            try:
                pokemon = reduce(
                    dict.__getitem__, [
                        'inventory_item_data', 'pokemon_data'], inv_data)
            except KeyError:
                pass
            else:
                if not pokemon.get(
                    ...
                    pokemon.get(
                        'favorite',
                        0) == 1 and self.config.get(
                        'dont_nickname_favorite',
                        False)):
    ...
    def _nickname_pokemon(self, pokemon):
        ...
        try:
            ...
            self.emit_event(
                'config_error',
                formatted="Unable to nickname {} due to bad template ({})".format(
                    name,
                    bad_key))
        if pokemon.get('nickname', '') == new_name:
            return
        response = self.bot.api.nickname_pokemon(
            pokemon_id=instance_id, nickname=new_name)
        sleep(1.2)
        try:
            result = reduce(
                dict.__getitem__, [
                    "responses", "NICKNAME_POKEMON"], response)
        except KeyError:
            self.emit_event(
                'api_error',
