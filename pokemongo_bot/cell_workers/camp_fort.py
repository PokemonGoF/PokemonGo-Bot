    ...
    def work(self):
        ...
        if self.destination is None:
            ...
            if len(forts_clusters) > 0:
                ...
                self.emit_event(
                    'new_destination',
                    formatted="*New destination* at ({}m): ({} forts, {} lured)".format(round(self.destination[4], 2), self.destination[3], self.destination[2])
                )
        ...
        if self.stay_until >= now:
            ...
            if self.no_log_until < now:
                self.emit_event(
                    ...
                    formatted="*Staying at destination*: ({} forts, {} lured)".format(cluster[3], cluster[2])
                )
                self.no_log_until = now + LOG_TIME_INTERVAL
            ...
            self.emit_event(
                ...
                formatted="*Arrived at destination*: ({} forts, {} lured)".format(cluster[3], cluster[2])
            )
            ...
            self.emit_event(
                ...
                formatted="*Moving to destination* at ({}m): ({} forts, {} lured)".format(round(cluster[4], 2), cluster[3], cluster[2])
            )
            self.no_log_until = now + LOG_TIME_INTERVAL
