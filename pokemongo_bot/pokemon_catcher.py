...
class PokemonCatcher(object):
    ...
    def catch(self):
        ...
        if not (response_dict and 'responses' in response_dict):
            time.sleep(5)
        return self.process_response(response_dict)

    def process_response(self, response_dict):
        encounter_id = self.pokemon['encounter_id']
        if self.response_key in response_dict['responses'] and \
                        self.response_status_key in response_dict['responses'][self.response_key] and \
                        response_dict['responses'][self.response_key][self.response_status_key] is 1:
            cp = 0
            pokemon_name = None
            pokemon_potential = None
            ...
            if not self.should_capture_pokemon(pokemon_name, cp, pokemon_potential):
            ...
            if self.check_vip_pokemon(pokemon_name, cp, pokemon_potential):
                logger.log(
                    '[-] {} is a VIP Pokemon! [CP {}] [Potential {}] Nice! Try our best to catch it!'.format(
                        pokemon_name, cp, pokemon_potential), 'red')
                flag_VIP = True
            ...
            while True:
                ...
                if flag_VIP:
                    if (berries_count > 0 and catch_rate[pokeball - 1] < 0.9):
                        ...
                        logger.log(
                            'Catch Rate with normal Pokeball is low ({}%). Thinking to throw a {}... ({} left!)'.format(
                                success_percentage, self.item_list[str(berry_id)], berries_count - 1))
                        ...
                        if response_dict and response_dict['status_code'] is 1 and 'item_capture_mult' in \
                            ...
                            berry_used = True
                            logger.log(
                                'Threw a berry! Catch Rate with normal Pokeball has increased to {}%'.format(
                                    success_percentage))
                        else:
                            if response_dict['status_code'] is 1:
                                logger.log('Fail to use berry. Seem like you are softbanned.', 'red')
                            else:
                                logger.log('Fail to use berry. Status Code: {}'.format(
                                    response_dict['status_code']), 'red')
    ...
    def should_capture_pokemon(self, pokemon_name, cp, iv):
