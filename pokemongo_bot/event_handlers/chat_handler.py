        ...
        if event == 'pokemon_caught':
            ...
            if trigger:
                if ((not "operator" in trigger or trigger["operator"] == "and") and data["cp"] >= trigger["cp"] and
                    ...
                    msg = "Captured {}! (CP: {} IV: {} A/D/S: {} NCP: {}) Catch Limit: ({}/{}) +{} exp".format(data["pokemon"], data["cp"], data["iv"], data["iv_display"], data["ncp"],  data["caught_last_24_hour"], data["daily_catch_limit"], data["exp"])
