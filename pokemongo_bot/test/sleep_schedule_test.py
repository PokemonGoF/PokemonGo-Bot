...
class SleepScheculeTestCase(unittest.TestCase):
    ...
    def test_get_next_time_called_near_activation_time(self, mock_datetime):
        mock_datetime.now.return_value = datetime(year=2016, month=8, day=01, hour=14, minute=51)
    ...
    @patch('pokemongo_bot.sleep_schedule.datetime')
    def test_start_within_sleep_range(self, mock_datetime):
    ...
    @patch('pokemongo_bot.sleep_schedule.datetime')
    def test_start_within_sleep_range_started_previous_day(self, mock_datetime):
        now = datetime(year=2016, month=8, day=1, hour=02, minute=00)
        mock_datetime.now.return_value = now
    ...
    def test_get_next_time_called_within_sleep_range(self, mock_datetime):
        now = datetime(year=2016, month=8, day=1, hour=16, minute=00)
        mock_datetime.now.return_value = now
    ...
    def test_call_work_before_schedule(self, mock_datetime, mock_sleep):
        ...
        mock_datetime.now.return_value = self.worker1._next_sleep - timedelta(minutes=5)
